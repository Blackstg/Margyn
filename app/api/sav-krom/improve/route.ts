// POST /api/sav-krom/improve
// Body: { current_draft: string }
// Asks Claude to improve the formulation of the current draft (Krom Water SAV).
...
export async function POST(req: NextRequest) {
  ...
  const prompt = `Voici une réponse rédigée par un agent SAV Krom Water (carafes filtrantes françaises).
...
Ne pas ajouter d'informations que tu ne connais pas. Ne pas supprimer d'informations existantes.
...
Réponds UNIQUEMENT avec la réponse améliorée, sans explication, sans markdown, sans backticks.`
  ...
  try {
    ...
    console.error('[SAV-Krom] improve error:', err)
