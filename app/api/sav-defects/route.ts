...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      shopify_order_id:    body.shopify_order_id?.toString().trim() || null,
...
export async function PATCH(req: NextRequest) {
  ...
  if (body.status !== undefined) {
    ...
    // Auto-renseigne la date d'envoi réclamation au passage à "reclamation_envoyee"
    if (body.status === 'reclamation_envoyee') {
      patch.claim_sent_at = body.claim_sent_at || new Date().toISOString().slice(0, 10)
    }
  }
  for (const f of ['reship_tracking_ref', 'received_at', 'claim_sent_at', 'supplier_claim_ref', 'notes'] as const) {
